...
function makeLogLines(
  ...
  } else if (status === "building_graph") {
    lines.push({ ts: now(), text: "文档解析完成", cls: "log-ok" });
    lines.push({ ts: now(), text: "正在抽取概念并构建图谱…", cls: "log-info" });
...
function statusToPhase(status: SessionStatus, chunkCount = 0): number {
  if (status === "uploaded" && chunkCount > 0) return 2;
  if (status === "uploaded") return 0;
  if (status === "ingesting") return 2;
  if (status === "building_graph") return 2;
...
export function PipelinePage() {
  ...
  const stats = statusData?.stats ?? session?.stats;
  ...
  useEffect(() => {
    ...
    setLocalStatus(null);
    getSession(id)
      .then((s) => setSession(s as CourseSession))
      .catch(() => {});
  }, [id]);
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setLocalStatus("building_graph");
      try {
        await buildGraph({ session_id: id! });
        setLocalStatus("graph_ready");
      } catch (e) {
        setLocalStatus(null);
  ...
  const logLines = useMemo(
    () => makeLogLines(currentStatus, errorMessage, stats?.chunk_count ?? 0),
    [currentStatus, errorMessage, stats?.chunk_count],
  );
  const isDone =
    currentStatus === "graph_ready" || currentStatus === "notes_ready";
    ...
    <div className="pipeline-page">
      ...
      <div className="pipeline-viz">
        ...
        <div className="viz-counters">
          <span>
            文档 <b>{stats?.document_count ?? "—"}</b>
          </span>
          <span>
            片段 <b>{stats?.chunk_count ?? "—"}</b>
          </span>
          <span>
            概念 <b>{stats?.concept_count ?? "—"}</b>
          </span>
        </div>
