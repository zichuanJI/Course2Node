...
- content_md 使用 Markdown；正文中重要术语加粗，列举用 - 分点（每条 1-2 句），对比内容用 Markdown 表格（列名加粗）。
- content_md 中不要使用任何 Markdown 标题语法（##、### 等），改用加粗 **...** 作为段内小标题。
...
整体结构：
- summary 是无标题的总览段落，概括本讲主线、各章节内容及建议学习路径，150-200字，写成连贯段落，不要分点或加标题。
- 之后每个章节一个 section，按知识递进顺序排列。

章节标题（title 字段）：
- 格式为“章节标题：副标题”，不加编号（不写“第X节”“一、”“1.”等）。
- 标题要像老师在组织一节课，优先使用“问题导向”或“主题导向”的标题，例如“马尔可夫链：如何描述状态转移”“虚拟内存：为什么需要地址空间抽象”，避免“其他概念”“补充内容”这类空泛标题。

每节正文格式（content_md 字段）：
- 以 1-2 句引入开头，说明本节定位或引出核心问题。
- 正文中重要术语必须加粗。
- 列举用 - 分点，每条 1-2 句，不要嵌套多层列表，每个分点必须不可再分。
- 对比内容用 Markdown 表格，列名加粗。
- 只用加粗 **...** 作为段内小标题（如 **本节核心问题**、**关键结论**），不要使用 ##、### 或任何 Markdown 标题语法，也不要使用编号列表（1. 2. 3.）。
- 每节末尾至少包含以下之一：**关键结论**、**学习路径**、**易混点**，用加粗标注后以 - 分点列出。
- 可选包含 **本节核心问题**。
- 直接从正文开始，末尾不写“以上就是本节内容”等空洞总结语。
...
- 如果多个概念容易混淆，优先用简短对比表把区别讲清楚；如果存在前置学习关系，优先写成递进链路；如果存在流程/机制，优先写成步骤或阶段。
...
- title 格式为“章节标题：副标题”，不加编号。
- 只返回 JSON object，格式为：
{"title":"章节标题：副标题","content_md":"Markdown正文","concept_ids":["concept:id1", "concept:id2"]}
"""

COURSE_SUMMARY_SYSTEM_PROMPT = """\
你是课程总笔记导读生成器。你会读取课程标题和已生成的章节标题，为整本课程总笔记生成一段总览段落。

要求：
- 写一段无标题的总览段落，概括本课程主线、各章节内容及建议学习路径，150-200字。
...
def _build_course_section_prompt(
    ...
    *,
    ...
    lines = [
        ...
        "",
        "输出结构：",
        '{"title":"章节标题：副标题","content_md":"","concept_ids":["concept:id"]}',
        "",
        "局部强化图谱：",
    ...
    lines.extend(
        [
            "",
            "生成要求：",
            "- title 格式为“章节标题：副标题”，聚焦本节核心概念，不加编号，不要使用泛泛的“课程总结/综合复习/核心概念”。",
            "- 只生成这一节，不要生成整本课程总览、导读或结语。",
            "- 正文以 1-2 句引入开头；用加粗 **...** 做段内小标题，不要使用 ## 或 ### 标题语法。",
...
def _build_notes_prompt(graph: GraphArtifact, *, lecture_title: str, topic: str = "") -> str:
    ...
    lines = [
        ...
        "- title 是这份笔记的标题，自然明确，像课程讲义标题，不加编号。",
        "- summary 是无标题的总览段落，概括本讲主线、各章节内容及建议学习路径，150-200字，写成连贯段落不分点。",
        "- sections 是主体内容；title 格式为“章节标题：副标题”不加编号；content_md 严格按上述格式规则书写，不能只写几句空泛概括。",
        "",
        "请输出 JSON：",
        '{"title":"","summary":"","sections":[{"title":"章节标题：副标题","content_md":"","concept_ids":["concept:id"]}]}',
        "",
        "图谱聚类：",
    ]
